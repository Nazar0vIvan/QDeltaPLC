...
QByteArray PlcMessageManager::buildRequestFrame(const QVariantMap& req, quint16 tid) const {
  quint8 cmd = 0;
  const QByteArray payload = buildReqPayload(req, cmd);
  if (!cmd) return {}; // unknown/missing cmd

  QByteArray frame = writeHeader(Type::REQ, tid, quint16(payload.size()));
...
QByteArray PlcMessageManager::buildReqPayload(const QVariantMap& req, quint8& outCmd) const {
  QByteArray body;
  ...
  if (cmd == "READ_IO") {
    ...
    ds << quint8(DEV::Y) << mod << andm << orm << quint16(0);
  }
  ...
  QByteArray payload; payload.reserve(2 + body.size());
...
QByteArray PlcMessageManager::writeHeader(Type type, quint16 tid, quint16 len) const {
  QByteArray out; out.reserve(8);
...
QVariantMap PlcMessageManager::parseRespOk(quint8 cmd, const QByteArray& body) const {
  ...
  switch (cmd) {
    case CMD::READ_IO: {
        if (body.size() != 3) return {{"type","PARSE_ERR"},{"error","READ_IO_BODY_SIZE"}};
        quint8 dev, mod, data; ds >> dev >> mod >> data;
        if (dev != DEV::X && dev != DEV::Y) return {{"type","PARSE_ERR"},{"error","READ_IO_BAD_DEV"}};
        m["cmdName"]="READ_IO"; m["dev"]=(dev==DEV::Y)?"Y":"X"; m["mod"]=int(mod); m["data"]=int(data);
        return m;
      }
    case CMD::READ_REG: {
        if (body.size() < 4) return {{"type","PARSE_ERR"},{"error","READ_REG_BODY_SHORT"}};
        quint8 dev,rsv; quint16 count; ds >> dev >> rsv >> count;
        const int expect = 4 + count*2;
        if (body.size() != expect) return {{"type","PARSE_ERR"},{"error","READ_REG_COUNT_MISMATCH"}};
        QVariantList regs; regs.reserve(count);
        for (quint16 i=0; i<count; i++) {
          quint16 w;
          ds >> w;
          regs << int(w);
        }
        m["cmdName"]="READ_REG"; m["count"]=int(count); m["data"]=regs;
        return m;
      }

    case CMD::WRITE_IO: {
        if (body.size() != 3) return {{"type","PARSE_ERR"},{"error","WRITE_IO_BODY_SIZE"}};
        quint8 dev, mod, newdata; ds >> dev >> mod >> newdata;
        m["cmdName"]="WRITE_IO"; m["mod"]=int(mod); m["newdata"]=int(newdata);
        return m;
      }

    case CMD::WRITE_REG: {
        if (body.size() != 4) return {{"type","PARSE_ERR"},{"error","WRITE_REG_BODY_SIZE"}};
        quint16 addr, count; ds >> addr >> count;
        m["cmdName"]="WRITE_REG"; m["addr"]=int(addr); m["count"]=int(count);
        return m;
      }

    case CMD::SNAPSHOT: {
        if (body.size() != 4) return {{"type","PARSE_ERR"},{"error","SNAPSHOT_BODY_SIZE"}};
        quint8 di1,do1,di2,do2; ds >> di1 >> do1 >> di2 >> do2;
        m["cmdName"]="SNAPSHOT"; m["di1"]=int(di1); m["do1"]=int(do1); m["di2"]=int(di2); m["do2"]=int(do2);
        return m;
      }
    default:
...
QVariantMap PlcMessageManager::parseRespErr(const QByteArray& payload) const {
  if (payload.size() != 4) return {{"type","PARSE_ERR"},{"error","RESP_ERR_BODY_SIZE"}};
  QBuffer b; b.setData(payload); b.open(QIODevice::ReadOnly);
  QDataStream ds(&b); ds.setByteOrder(QDataStream::LittleEndian);
  quint8 cmd, err; quint16 code; ds >> cmd >> err >> code;
